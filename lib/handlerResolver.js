...
function createResolver(args){
    ...
    const resolveRequestHandler = function (request, response) {
        ...
        return new Promise((defResolve,defReject) => {
            ...
            console.info("" + request.method + " " + wrappedReq.getPathname())
                ...
                switch (wrappedReq.getMethod()) {
                    case 'GET':
                        if (wrappedReq.containsQueryParam('uri')) {
                            ...
                            fs.stat(requestedFilePath).then(function (stat) {
                                ...
                                if (urlPathExtname === '.html' || !urlPathExtname) {
                                    ...
                                    fs.stat(lessFp).then(function (stat) {
                                        if (stat.isFile()) {
                                        ...
                                        fs.stat(scssFp).then(function (stat) {
                                            let h;
                                            if (stat.isFile()) {
