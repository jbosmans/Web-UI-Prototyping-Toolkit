...
function downloadImagesFromFlickr(tags){
    return new Promise((resolve,reject) => {
        var tagsString = typeof tags === 'string' ? tags : tags.map((t) => encodeURIComponent(t)).join(",");
        ...
        utils.downloadJsonpData(opt, function(err, data){
            if(err){
                console.error("Error getting flickr photos: " + err);
                reject(err);
            }else{
                let items = [].concat(data.items);
...
function downloadNasaImageOfTheDay(){
    return new Promise((resolve,reject) => {
...
module.exports =  function (rc) {
    ...
    downloadImagesFromFlickr(tagsString).then((items) => {
        items.forEach(function(i){
            i.tagsString = tagsString;
            i.id = imageCache.keyFn(i);
        });

        imageCache.storeAll(items);
        let imagesFound = items.length > 0;
