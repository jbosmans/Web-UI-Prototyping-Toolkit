...
const defaultDropPointPrefix = '<!-- ';
const defaultDropPointPostfix = ' -->';
const defaultDropPointSeparatorName = ':';
const defaultDropPointSeparatorArgs = ',';
...
class TemplateComposer {
     ...
     * @param {number} maxCompilationRuns
     * @param {string} dropPointPrefix
     * @param {string} dropPointPostfix
     * @param {string} dropPointSeparatorName
     * @param {string} dropPointSeparatorArgs
     */
    constructor({
        runtime,
        maxCompilationRuns = 100,
        dropPointPrefix = defaultDropPointPrefix,
        dropPointPostfix = defaultDropPointPostfix,
        dropPointSeparatorName = defaultDropPointSeparatorName,
        dropPointSeparatorArgs = defaultDropPointSeparatorArgs
    }) {
        this.maxCompilationRuns = maxCompilationRuns;
    ...
    loadPluginFactories() {
        ...
        const plugins = {};
        const fileNames = this.runtime.listDir(path.resolve(__dirname, "fotl"));
        fileNames.forEach(fn =>{
            if(fn.indexOf("fotl-") === 0 && fn.substring(fn.length-3) === ".js"){
                const pluginName = fn.substring(5, fn.length - 3);
                let fotlPlugin = require("./fotl/fotl-" + pluginName);
                let fotlPluginFactory = fotlPlugin.createFactory({
                    runtime: this.runtime,
                    composer: this
                });
                plugins[pluginName] = fotlPluginFactory;
