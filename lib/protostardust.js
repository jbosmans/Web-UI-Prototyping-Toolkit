...
const
    ...
    lessCssSupport = require('./lessCssSupport'),
    ...
    opn = require('opn'),
    ServerSideLessCompiler = require('./ServerSideLessCompiler'),
    StaticBuilder = require('./StaticBuilder');
...
class ProtoStarServer {
    ...
    constructor({runtime/*, minResponseDelay = 0, maxResponseDelay = 0*/}) {
        ...
        this.sslc = new ServerSideLessCompiler(this.runtime.constructProjectPath(""), () => this.project["lessParserAdditionalArgs"]);//lessCompiler.createServerSideLessCompiler(this.runtime.constructProjectPath(""), () => this.project["lessParserAdditionalArgs"]);
    ...
    buildPrototype(targetDir, callBack) {
        this.projectBuilder = new StaticBuilder({
            project: this.project,
            runtime: this.runtime,
            composer: this.composer
        });
    ...
    requestHandler(request, response, self) {
        ...
        if (prjCfg.hasOwnProperty('runtime') && prjCfg.runtime.hasOwnProperty('auth') && prjCfg.runtime.auth.hasOwnProperty('users') && prjCfg.runtime.auth.users.length) {
            ...
            if (credentials) {
                let ok = false;
                let c = credentials;
                prjCfg.runtime.auth.users.forEach(u =>{
