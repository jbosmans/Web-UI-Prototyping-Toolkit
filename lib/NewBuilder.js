    ...
    postProcessComposed(markup) {
        ...
        return new Promise((resolve, reject) => {
            if (markup.content.trim().length > 1) {
                jqueryRunner.runJQuery(markup.content, function modifyBuiltMarkupWithJQuery($) {
                        ...
                        jqueryRunner.processProtostarAttributes($, function (attrName, attrVal) {
                            return self._runtime.determineProtostarAttributeValue(attrName, attrVal, self._targetDir);
                        });
...
//return doctype + '\n' + window.document.documentElement.outerHTML; //$("html")[0].outerHTML;
    ...
    copyDependencyDirs(absoluteTargetDepUrl, copiedDirPathsMap) {
        ...
        if(absoluteTargetDepUrl.indexOf("http://") === 0){
    ...
    postProcessComposedForRelative(markup) {
        ...
        return new Promise((resolve,reject) => {
            if (markup.content.trim().length > 1) {
                jqueryRunner.runJQuery(markup.content, function modifyBuiltMarkupToRelativeWithJQuery($, window, metadata) {
                        try {
                            jqueryRunner.assignUniqueIdsToEditables($);
    ...
    makeBuildRelative(compiledTemplates, callBack) {
        ...
        const relativizeCompiledTemplate = function (templatePath) {
            return new Promise((resolve, reject) => {
                ...
                self.postProcessComposedForRelative(ct.pageContentsCompiled).then(function (contentsPostProcessedRelative) {
                    resolve({
                        path: templatePath,
    ...
    compileTemplate(pagePath, pageContents) {
        ...
        return new Promise((resolve, reject) => {
            ...
            self.postProcessComposed(pageContentsCompiled).then(function (pageContentsPostProcessed) {
    ...
    buildPrototype() {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.compileTemplates(pagesToCompile).then((compiled) => {
                ...
                this.afterPostProcessing(compiledTemplates, function () {
